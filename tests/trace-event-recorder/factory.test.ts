...
describe('createRecorder', () => {
  it('writes a meta event on reset() with merged static + dynamic context', () => {
    const { rt, sink } = buildHarness({ schemaVersion: 1, pid: 1234 });
  ...
  it('pairs __enter with __ok (seq, enterSeq, depth, duration)', () => {
    const { rt, sink, clock } = buildHarness();
    rt.reset();
    rt.__enter(meta());
    clock.advance(5);
    rt.__ok();
  ...
  it('emits a throw event with serialized error when __throw is called', () => {
    ...
    const throwEvt = sink.snapshot().find((e: any) => e.type === 'throw');
  ...
  it('serializes variable snapshots passed to __ok', () => {
    ...
    const ok = sink.snapshot().find((e: any) => e.type === 'ok');
  ...
  it('includes step context on every enter event (push/pop balanced)', () => {
    ...
    rt.__enter(meta());
    rt.__ok();
    const events = sink.snapshot().filter((e: any) => e.type === 'enter');
    expect(events[0].step).toBe('login');
    expect(events[0].stepPath).toEqual(['login']);
    expect(events[1].step).toBe(null);
    expect(events[1].stepPath).toBe(null);
  });
  ...
  it('reads page.url() via setPage for the pageUrl field', () => {
    ...
    rt.__enter(meta());
    const enter = sink.snapshot().find((e: any) => e.type === 'enter');
  ...
  it('reset() clears previous events via sink.clear()', () => {
    const { rt, sink } = buildHarness();
    rt.reset();
    rt.__enter(meta());
    rt.__ok();
    expect(sink.snapshot().length).toBeGreaterThan(1);
    rt.reset();
  ...
  it('setCurrentStatementScreenshot is a no-op when the stack is empty', () => {
    ...
    const types = sink.snapshot().map((e: any) => e.type);
