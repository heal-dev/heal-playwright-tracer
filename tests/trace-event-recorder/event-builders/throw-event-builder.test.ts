...
import { buildThrowEvent } from '../../../src/trace-event-recorder/event-builders/throw-event-builder';
...
const lastEvent = (state: { sink: { snapshot(): any[] } }) => {
  const snap = state.sink.snapshot();
  return snap[snap.length - 1];
};

describe('buildThrowEvent', () => {
  it('writes an event of type "throw" to the sink', () => {
