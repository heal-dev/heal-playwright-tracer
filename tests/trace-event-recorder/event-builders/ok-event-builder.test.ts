...
import { buildEnterEvent } from '../../../src/trace-event-recorder/event-builders/enter-event-builder';
import { buildOkEvent } from '../../../src/trace-event-recorder/event-builders/ok-event-builder';
import { createTestRecorderState, createFakeClock } from '../../helpers/trace-event-recorder-state';
import type { EnterMeta } from '../../../src/trace-event-recorder/state';
...
describe('buildOkEvent', () => {
  it('writes an event of type "ok" to the sink', () => {
    const state = createTestRecorderState();
    buildEnterEvent(state, sampleMeta());
    buildOkEvent(state);
    const types = state.sink.snapshot().map((e) => (e as { type: string }).type);
  ...
  it('stamps seq = current + 1 and increments state.seq', () => {
    ...
    const ok = state.sink.snapshot()[1] as unknown as { seq: number };
  ...
  it('sets enterSeq to the matching enter event seq', () => {
    const state = createTestRecorderState();
    buildEnterEvent(state, sampleMeta());
    const enterSeq = (state.sink.snapshot()[0] as unknown as { seq: number }).seq;
    buildOkEvent(state);
    const ok = state.sink.snapshot()[1] as unknown as { enterSeq: number };
  ...
  it('computes duration = (now - startedAt) - enter.t', () => {
    ...
    clock.advance(17);
    buildOkEvent(state);
    const ok = state.sink.snapshot()[1] as unknown as { duration: number };
  ...
  it('attaches a serialized `vars` snapshot when vars are provided', () => {
    ...
    const ok = state.sink.snapshot()[1] as unknown as { vars?: Record<string, unknown> };
  ...
  it('omits `vars` entirely when no vars are passed', () => {
    ...
    buildEnterEvent(state, sampleMeta());
    buildOkEvent(state);
    const ok = state.sink.snapshot()[1] as unknown as Record<string, unknown>;
  ...
  it('is a silent no-op when the stack is empty (orphan ok)', () => {
    const state = createTestRecorderState();
    buildOkEvent(state);
    expect(state.sink.snapshot()).toHaveLength(0);
  ...
  it('stamps wallTime as a finite number (Date.now)', () => {
    ...
    const ok = state.sink.snapshot()[1] as unknown as { wallTime: number };
