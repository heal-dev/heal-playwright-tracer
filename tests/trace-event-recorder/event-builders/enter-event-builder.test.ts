...
// Every test pokes at sink.snapshot()[0] as `any` to dodge the
...
describe('buildEnterEvent', () => {
  it('writes an event of type "enter" to the sink', () => {
    const state = createTestRecorderState();
    buildEnterEvent(state, sampleMeta());
    expect(state.sink.snapshot()).toHaveLength(1);
  ...
  it('reads depth and parentSeq from the stack BEFORE push', () => {
    ...
    const inner = state.sink.snapshot()[1] as unknown as Record<string, unknown>;
