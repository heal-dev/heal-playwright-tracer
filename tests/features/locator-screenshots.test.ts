...
// Mock the recorder entrypoint before importing locator-screenshots so
// the module-load side effects (which pull in the real recorder and
// install globalThis hooks) are replaced by a spy. The spy lets every
// test assert exactly which filename was stamped onto the active
// statement without depending on a live recorder instance.
vi.mock('../../src/trace-event-recorder/entrypoint', () => ({
  setCurrentStatementScreenshot: vi.fn(),
}));

import { startLocatorScreenshotCapture, wrapExpect } from '../../src/features/locator-screenshots';
import { setCurrentStatementScreenshot } from '../../src/trace-event-recorder/entrypoint';

const mockSetScreenshot = vi.mocked(setCurrentStatementScreenshot);
...
describe('locator-screenshots', () => {
  ...
  it('runs boundingBox → overlay → screenshot → action → remove overlay in order', async () => {
    ...
    startLocatorScreenshotCapture(fakePage as never, '/tmp/out');
  ...
  it('skips capture entirely after the session ends', async () => {
    const { fakePage, FakeLocator, screenshotPaths } = makeFakePageAndLocatorClass();
    const stop = startLocatorScreenshotCapture(fakePage as never, '/tmp/out');
    stop();
